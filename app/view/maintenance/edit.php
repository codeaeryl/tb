...
<main style="position: relative;">
    ...
    <div class="container">
        <h1>Edit Maintenance <?= $maintenance->getIdMaintenance() ?></h1>
        <form action="index.php?menu=maintenance-update" method="POST">
            <?php if (isset($_GET['message'])): ?>
                <div class="err-msg"><?= htmlspecialchars($_GET['message']) ?></div>
            <?php endif; ?>
            <input type="hidden" name="id_maintenance" value="<?= $maintenance->getIdMaintenance() ?>">
            
            <label for="nomor_kamar">Nomor Kamar:</label>
            <input type="text" id="nomor_kamar" name="nomor_kamar" value="<?= $maintenance->getNomorKamar() ?>" required>
            
            <label for="tanggal_mulai">Tanggal Mulai:</label>
            <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="<?= $maintenance->getTanggalMulai() ?>" required>
            
            <label for="tanggal_selesai">Tanggal Selesai:</label>
            <input type="date" id="tanggal_selesai" name="tanggal_selesai" value="<?= $maintenance->getTanggalSelesai() ?>" required>
            
            <label for="deskripsi_maintenance">Deskripsi:</label>
            <textarea id="deskripsi_maintenance" name="deskripsi_maintenance" required><?= $maintenance->getDeskripsiMaintenance() ?></textarea>
            
            <label for="status_maintenance">Status:</label>
            <select id="status_maintenance" name="status_maintenance">
                <option value="Scheduled" <?= $maintenance->getStatusMaintenance() == 'Scheduled' ? 'selected' : '' ?>>Scheduled</option>
                <option value="Completed" <?= $maintenance->getStatusMaintenance() == 'Completed' ? 'selected' : '' ?>>Completed</option>
            </select>
